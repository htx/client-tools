...
void EditableAnimationState::removeChildState(EditableAnimationState *child)
{
	...
	{
		WARNING(true, ("removeChildState(): specified child [0x%x] is not a child.", reinterpret_cast<intptr_t>(child)));
		return;
	}
...
void EditableAnimationState::removeLink(EditableAnimationStateLink *link)
{
	...
	if (findIt == m_stateLinks.end())
		...
		WARNING(true, ("tried to remove EditableAnimationStateLink (0x%x) but state doesn't know about link.", reinterpret_cast<intptr_t>(link)));
}
