...
namespace Direct3d11_RenderTargetNamespace
{
	...
	ID3D11Texture2D		  *ms_primarySurface;
	ID3D11Texture2D       *ms_primaryDepthStencilSurface;

	ID3D11Texture2D       *ms_systemMemoryTexture;
	ID3D11Texture2D       *ms_systemMemorySurface;

	ID3D11Texture2D       *ms_renderTargetTexture;
	ID3D11Texture2D       *ms_renderSurface;

	ID3D11Texture2D       *ms_userSurface;
...
void Direct3d11_RenderTarget::restoreDevice()
{
	/*HRESULT hresult;

	ID3D11Device *device = NON_NULL(Direct3d11::getDevice());
...
void Direct3d11_RenderTarget::lostDevice()
{
	DEBUG_FATAL(!ms_primaryTargetSet, ("Direct3d11_RenderTarget::lostDevice() designed to be called outside of beginTextureFrame/endTextureFrame pair"));
...
void Direct3d11_RenderTarget::setRenderTarget(Texture *texture, CubeFace cubeFace, int mipmapLevel)
{
	...
	HRESULT hresult;
	ID3D11Device * const device = NON_NULL(Direct3d11::getDevice());
	...
	if (!texture->isRenderTarget())
	{
		...
		ID3D11Texture2D * d3dBaseTexture = safe_cast<Direct3d11_TextureData*>(texture->m_graphicsData)->getBaseTexture();
		if (cubeFace == CF_none)
		{
			hresult = static_cast<ID3D11Texture2D *>(d3dBaseTexture)->GetSurfaceLevel(mipmapLevel, &ms_userSurface);
		//	FATAL_DX_HR("GetSurfaceLevel() failed for render d3dTexture %s", hresult);
		}
		else
		{
			hresult = static_cast<ID3D11Texture3D *>(d3dBaseTexture)->GetCubeMapSurface(Direct3d11::getD3dCubeFace(cubeFace), mipmapLevel, &ms_userSurface);
		//	FATAL_DX_HR("GetSurfaceLevel() failed for render d3dTexture %s", hresult);
		}
	...
	{
		ID3D11Texture2D * d3dBaseTexture = safe_cast<Direct3d9_TextureData*>(texture->m_graphicsData)->getBaseTexture();

		if (cubeFace == CF_none)
		{
			hresult = static_cast<ID3D11Texture2D *>(d3dBaseTexture)->GetSurfaceLevel(mipmapLevel, &ms_renderSurface);
			FATAL_DX_HR("GetSurfaceLevel() failed for render d3dTexture %s", hresult);
		}
		else
		{
			hresult = static_cast<ID3D11Texture3D *>(d3dBaseTexture)->GetCubeMapSurface(Direct3d9::getD3dCubeFace(cubeFace), mipmapLevel, &ms_renderSurface);
			FATAL_DX_HR("GetSurfaceLevel() failed for render d3dTexture %s", hresult);
		}
	}
...
bool Direct3d11_RenderTarget::copyRenderTargetToNonRenderTargetTexture()
{
	...
	{
		// get the device
		ID3D11Device *device = NON_NULL(Direct3d9::getDevice());
