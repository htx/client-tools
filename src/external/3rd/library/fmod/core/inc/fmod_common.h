...
typedef struct FMOD_REVERB_PROPERTIES
{
	...
	bool operator==(const FMOD_REVERB_PROPERTIES& other) const {
    return DecayTime == other.DecayTime && EarlyDelay == other.EarlyDelay && LateDelay == other.LateDelay && HFReference == other.HFReference && HFDecayRatio == other.HFDecayRatio
						&& Diffusion == other.Diffusion && Density == other.Density && LowShelfFrequency == other.LowShelfFrequency && LowShelfGain == other.LowShelfGain
						&& HighCut == other.HighCut && EarlyLateMix == other.EarlyLateMix && WetLevel == other.WetLevel;
}
