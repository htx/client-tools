...
void SwgCuiQuestBuilder::createSlotMap()
{
	...
	for(unsigned int i = 0; i < pageCollections.size(); ++i)
	{
		...
		for(unsigned int j = 0; j < collectionSlots.size(); ++j)
		{
			...
			{
				WARNING(true, ("Failed to add slot %s with the CRC %d to our slot map.", newSlotInfo.slotName.c_str(), newSlotInfo.slotCrc.getCrc()));
			}
			...
			{
				WARNING(true, ("Failed to add slot %s with the CRC %d to our Name-To-Slot map.", newSlotInfo.slotName.c_str(), newSlotInfo.slotCrc.getCrc()));
			}
		}
	}
