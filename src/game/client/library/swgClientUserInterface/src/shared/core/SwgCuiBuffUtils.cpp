...
UIImage * SwgCuiBuffUtils::addBuffIcon(const Buff & buff, UIVolumePage & bufPage, const UIImage & sampleStateIcon, bool const isPlayerHamBar, bool const isGod)
{
	...
	{
		UIImageStyle * imageStyle2 = safe_cast<UIImageStyle *>(UIManager::gUIManager().GetRootPage()->GetObjectFromPath (UIString(Unicode::narrowToWide("/Styles.Icon.mood.careful")), TUIImageStyle));

		if (imageStyle2)
		{
			image->SetStyle(imageStyle2);
		}
	}
